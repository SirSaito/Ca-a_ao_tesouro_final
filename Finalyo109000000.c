...
void print_mina_result(void);

int x, y, n, t, q, i, j, k, c, b=0;
int X, Y;
char denovo;
float diff;
int n;
...
void ordem(void){
   while(num_jog==2){
        if(b==0){
            ranking();
            printf("\t\tVez do jogador número 1: %c\n", nome1);
            b++;
            posicao ();
        }
        if(b==1){
            ranking();
            printf("\t\tVez do jogador número 2: %c\n", nome2);
            b=0;
            posicao ();
        }
   }
   while(num_jog==3){
        ...
        if(b==2){
            ranking();
            printf("\t\tVez do jogador número 3: %c\n", nome3);
            b==0;
            posicao ();
        }
    }  
   while(num_jog==4){
        ...
        if(b==3){
            ranking();
            printf("\t\tVez do jogador número 4: %c\n", nome4);
            b==0;
            posicao ();
        }
    }  
...
void grid(void){
    int i = 0, j = 0;
     ...
     while( j < X )                                          //Nested loop for making the blank minefield and final minefield
    {
        while( i < Y)
        {
            Mina[i][j] = '-';
         ...
         while( j < X )                                          
    {
        while( i < Y)
        {
            ...
            if((Mina[i-1][j-1] == '_') && (Mina[i][j] != '_'))
            {
                Mina[i][j]++;
            }
            if((Mina[i-1][j] == '_') && (Mina[i][j] != '_'))
            {
                Mina[i][j]++;
            }
            if((Mina[i][j-1] == '_') && (Mina[i][j] != '_'))
            {
                Mina[i][j]++;
            }
            if((Mina[i-1][j+1] == '_') && (Mina[i][j] != '_'))
